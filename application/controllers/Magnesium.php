...
class Magnesium extends CI_Controller
{
    ...
    {
        $config['upload_path'] = 'assets/geojson/4magnesium/';
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!$this->upload->do_upload('fileupload')) {
                ...
                $this->db->insert('magnesium_file', $data);
    ...
    public function edit()
    {
        ...
        $result = $this->M_datamanager->ambil($id, 'magnesium_file');
    ...
    public function aktif($id)
    {
        ...
        $ganti = ['status' => 0];
        $this->M_datamanager->aktif($status, $ganti, 'magnesium_file');
        /* cari id update */
        $where = array('id' => $id);
        $data = ['status' => 1];
        /* selesai */
        $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">data berhasil diaktifkan!</div>');
        $this->M_datamanager->replace($where, $data, 'magnesium_file');
        redirect('Admin/save_data');
    }
    ...
    public function delete_data($id)
    {
        ...
        /* hapus file */
        $where = array('id' => $id);
        $result = $this->M_datamanager->ambil_file($where, 'magnesium_file');
        $url = $result['url'];
        $path = 'assets/geojson/4magnesium/';
        unlink($path . $url);
        /* hapus database */
        $this->M_datamanager->trash($where, 'magnesium_file');
